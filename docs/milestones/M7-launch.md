...
- **The item list is supplied by the product owner** ([Q9](../DECISIONS.md)), not written by an implementation agent. This milestone builds the loader, the warm-up and the marking; it does not invent content.
- **Sizing check before launch** ([§11.4](../SPEC.md#114-cold-start-and-the-catalogue-floor)): at least 50 items, 150–300 preferred, across at least 8 domains with no domain above 20%. If the supplied list is smaller, **do not silently ship the default cooldowns** — `USER_PAIR_COOLDOWN_DAYS` and `USER_RECENT_ITEMS` must be scaled down from the actual item count, or heavy players hit `503 no_items_available` in their first session. Compute and log the derived values at boot.
