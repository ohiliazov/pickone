...
- **The item list is supplied by the product owner** ([Q9](../DECISIONS.md)), not written by an implementation agent. It is already committed at [`seed/items.txt`](../../seed/items.txt) — **100 one-or-two-word items, validated against [§12.3](../SPEC.md#123-structural-validation) with zero failures, zero duplicates and zero slug collisions** ([`seed/README.md`](../../seed/README.md) has the analysis). This milestone builds the loader, the warm-up and the marking; it does not invent content.
...
- **The 8-domain rule passes**: 12 domains, the largest at 22% and only because `mundane` is a catch-all (taxes, socks, dentists, mondays share nothing but ordinariness). The items share no single comparison axis, so matchups stay absurd by construction.
